...
const (
	compactInterval  = 60 * time.Minute // Incremented compact interval to 60 minutes
...
/**
 * Sets up the SQLLog struct, linking it to the dialect.
 * Used by psql.go when the database is created.
 */
...
/**
 * Launches the compaction mechanism
 */
 ...
 * It includes rollback mechanisms in SQL. In DDS they don't exist.
 */
func (s *SQLLog) compactStart(ctx context.Context) error {
	...
	_, _, events, err := RowsToEventsDds(rows)
	if err != nil {
		return err
	}
	...
	// TODO: BeginTx calls to tx.go. Unusable for DDS
...
func (s *SQLLog) After(ctx context.Context, prefix string, revision, limit int64) (int64, []*server.Event, error) {
	...
	rev, compact, result, err := RowsToEventsDds(rows)
...
func (s *SQLLog) List(ctx context.Context, prefix, startKey string, limit, revision int64, includeDeleted bool) (int64, []*server.Event, error) {
	var (
		// Changed rows from SQL to DDS
		rows []*messagestructures.QueryGenericRow
		err  error
	)
...
/**
 * Implements the same logic as the default RowsToEvents but for DDS rows
 */
func RowsToEventsDds(rows []*messagestructures.QueryGenericRow) (int64, int64, []*server.Event, error) {
	var (
		result  []*server.Event
		rev     int64
		compact int64
	...
	for _, elem := range rows {
		// PrevKV can take different values depending on the created field
		// Therefore, a placeholder is created
		var selectedPrevKV *server.KeyValue

		// Populate CurrentKv accordingly
		currentKv := server.KeyValue{
			Key:            elem.Name,
			CreateRevision: elem.CreateRevision,
			ModRevision:    elem.Theid,
			Value:          []byte(elem.Value),
			Lease:          elem.Lease,
		}

		// Checking if element is new. If it's new, PrevKV is nil
		if 0 != elem.Created {
			selectedPrevKV = nil
			// For some reason, when created is true, kine doesn't bother to store in the database that create_revision is the same id.
			// It may be due to the use of a serial for the id (?). Regardless of the reason, this is done here.
			currentKv.CreateRevision = elem.Theid
		} else {
			// Populate PrevKv accordingly
			selectedPrevKV = &server.KeyValue{
				Key:            "",
				CreateRevision: 0,
				ModRevision:    elem.PrevRevision,
				Value:          []byte(elem.OldValue),
				Lease:          0,
			}
		}
		// Populate the whole Event structure
		event := &server.Event{
			Delete: 0 != elem.Deleted,
			Create: 0 != elem.Created,
			KV:     &currentKv,
			PrevKV: selectedPrevKV,
		}
		result = append(result, event)
	}

	// Fill the maxId in rev. It is the same value for all rows, pick the first one
	if len(rows) > 0 {
		rev = rows[0].MaxId
		compact = rows[0].CompactionPrevRevision
	} else {
		// Ensure that it returns nothing if it finds nothing (although it shouldn't be the case)
		var emptyEvents []*server.Event
		return 0, 0, emptyEvents, nil
	}
