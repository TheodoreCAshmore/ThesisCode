...
func (l *LogStructured) Create(ctx context.Context, key string, value []byte, lease int64) (revRet int64, errRet error) {
	...
	rev, prevEvent, err := l.get(ctx, key, "", 1, 0, true) // Uses List from sql.go
	if err != nil {
		return 0, err
	}
	...
	revRet, errRet = l.log.Append(ctx, createEvent) // Inserts a new row in the DB
	return
}
...
func (l *LogStructured) Delete(ctx context.Context, key string, revision int64) (revRet int64, kvRet *server.KeyValue, deletedRet bool, errRet error) {
	...
	rev, err = l.log.Append(ctx, deleteEvent) // Inserts a new Row in the DB with the deletion of the key
	if err != nil {
